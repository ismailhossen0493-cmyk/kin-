        ...
        <div className="max-w-6xl mx-auto">
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
            ...
            <div className="flex gap-4">
              {activeTab === 'inventory' && (
                <button 
                  onClick={() => setIsEditingProduct({ name: '', price: 0, category_id: categories[0]?.id, brand: '', stock: 0, image_url: '', description: '', sizes: [], colors: [], product_code: '' })}
                            ...
                            {JSON.parse(order.items).map((item: any, idx: number) => (
                              <div key={idx} className="text-[10px] font-medium text-zinc-600 bg-zinc-50 p-2 rounded-lg border border-black/5">
                                <div className="flex justify-between items-start mb-1">
                                  ...
                                  {item.product_code && <span className="text-[8px] bg-zinc-200 px-1 rounded font-mono">{item.product_code}</span>}
                                </div>
                                <div className="flex gap-2 text-zinc-400">
                                  ...
                                  {item.color && (
                                    <div className="flex items-center gap-1">
                                      <span>• Color:</span>
                                      <div className="w-2 h-2 rounded-full border border-black/10" style={{ backgroundColor: item.color }} />
                                    </div>
                                  )}
                                </div>
                              </div>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {product.product_code && <span className="text-[8px] bg-zinc-100 px-1 rounded font-mono text-zinc-500">{product.product_code}</span>}
                      </div>
                    </div>
                    ...
                    <tr key={user.id}>
                      ...
                      <td className="px-8 py-6">
                        <select 
                          ...
                          className="bg-zinc-100 border-none rounded-xl text-xs font-bold px-3 py-2 outline-none focus:ring-2 focus:ring-primary/20"
                        >
              ...
              <form onSubmit={handleProductSubmit} className="p-12 space-y-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                  <div className="space-y-10">
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-4 block">Product Code / SKU</label>
                      <input className="w-full p-5 bg-zinc-50 border border-black/5 rounded-2xl outline-none focus:ring-2 focus:ring-primary/20 transition-all font-mono text-xs" value={isEditingProduct.product_code || ''} onChange={e => setIsEditingProduct({...isEditingProduct, product_code: e.target.value})} placeholder="e.g. KIN-PAN-001" />
                    </div>
                    ...
                    <div className="flex items-center gap-4 p-5 bg-zinc-50 border border-black/5 rounded-2xl">
                      <input 
                        type="checkbox" 
                        id="preorder"
                        className="w-5 h-5 rounded border-black/10 text-primary focus:ring-primary/20"
                        checked={isEditingProduct.is_preorder === 1}
                        onChange={e => setIsEditingProduct({...isEditingProduct, is_preorder: e.target.checked ? 1 : 0})}
                      />
                      <label htmlFor="preorder" className="text-xs font-bold text-zinc-900 uppercase tracking-widest cursor-pointer">Enable Pre-order Mode</label>
                    </div>
                    <div>
